...
/*! If you created MMetaConfigEditor without providing MMetaConfig object you need to call
 * this method to prepare layout for \a config. It can be done only once for each object.
 * If you want to edit another config created different editor object.
 */
void MMetaConfigEditor::setConfig(MMetaConfig *config)
{
    Q_ASSERT_X(!m_config, "MMetaConfigEditor",
               "MMetaConfig object can be loaded only once!");
    m_config = config;
...
{
    Q_ASSERT_X(m_config, "MMetaConfigEditor", "MMetaConfig is not set. "
               "Pass it as argument to c-tor or via setConfig.");
